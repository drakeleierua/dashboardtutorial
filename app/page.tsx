...
import CustomBackground from "@/components/bg"; // Adjust the import path as needed
...
export default async function DashBoard() {
  ...
  const monthlyUsersData = eachMonthOfInterval({
    ...
    const userMonthly = usersThisMonth.filter(user => format(new Date(user.createdAt), 'MMM') === monthString)
  ...
  const monthlySalesData = eachMonthOfInterval({
    ...
    const monthString = format(month, 'MMM');
    const salesInMonth = salesThisMonth.filter(sale => format(new Date(sale.createdAt), 'MMM') === monthString)
